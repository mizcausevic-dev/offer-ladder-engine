...
$stdout = Join-Path $env:TEMP ("offer-ladder-engine-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("offer-ladder-engine-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5310"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/offer-paths"; File = "02-offer-paths-proof.png"; Size = "1600,1420" },
        @{ Url = "http://127.0.0.1:$port/conversion-branches"; File = "03-conversion-branches-proof.png"; Size = "1600,1340" },
